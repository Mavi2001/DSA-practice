...
int main()
{
    ...
    for(int value : v) //range based loops,   here the values stored in "value" are copied not actual.
    {
        cout<<value<<" ";
    }
    cout<<endl;
    for(int &value : v) //here reference is used, so the values in "value" are its own values
    {
        value++;
    }
    for(int &value : v)
